...
prism_tmax_0701_KS_sr <- crop(prism_tmax_0701_sr, KS_county_sf)
```
...
Oh, okay. So, the problem is that the points data (`KS_wells`) is an `sf` object, and `terra::extract()` does not like it (I thought this would be fixed by now after a year...). Let's turn KS_wells into an `SpatVect` object and then apply `terra::extract()`:
...
```{r extract-raster-sf, echo = F}
# saveRDS(tmax_from_prism, "Data/tmax_from_prism.rds")
tmax_from_prism <- readRDS("Data/tmax_from_prism.rds")
```
