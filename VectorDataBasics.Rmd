...
+ download the pertinent datasets from [here](https://www.dropbox.com/sh/c2mxn7bfxepd3wm/AAB77AgOnaCg27gosbAJvrLKa?dl=0) 
...
```{r interactove-nc, fig.cap = "Interactive map of North Carolina counties", cache = F, eval = F}
...
```{r nc-import-again, fig.cap = "Interactive map of North Carolina counties", cache = F, echo = F, results = "hide"}
#--- read the NC county map data ---#
nc <- st_read(system.file("shape/nc.shp", package="sf")) 
```
...
When you convert an `sf` of polygons into a `data.table`, then the geometry column appears to have lost the geometry information as all the entries are just `<XY>`.   
