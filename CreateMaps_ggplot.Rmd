...
+ download the pertinent datasets from [here](https://www.dropbox.com/sh/remr9jxqdpki5au/AAAAIDP9WN5mn4-5UhBMyssba?dl=0) 
...
The `nudge_x` and `nudge_y` options let you shit the labels.
...
tmax_Jan_09_df <- as.data.frame(as(tmax_Jan_09_rs, "Raster"), xy = TRUE) %>% 
...
```{r tmax-long}
tmax_long_df <- pivot_longer(tmax_Jan_09_df, c(-x, -y), names_to = "date", values_to = "tmax")  
...
Two types of `scale` functions are offered by the packages:
...
+ create a map of the area of interest, turn it into a grob using `ggplotGrob()`
+ create a map of the region that includes the area of interest
...
```{r eval = F}
ggsave(g_nc, file = "nc.pdf", height = 5, width = 7)
```
