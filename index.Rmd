...
+ **Bowen Chen**, Postdoctoral research, Department of Agricultural and Consumer Economics, University of Illinois at Urbana-Champaign
	* Section \@ref(CropScapeR)
+ **Shinya Uryu**, Research stuff, National Institute of Environmental Studies, Japan  
	* Section \@ref(ggsave)
...
+ Chapter 4: The basics of raster data handling using the `raster` and `terra`  packages (available)
...
+ Chapter 6: Speed things up (available)  
	* make raster data extraction faster by parallelization
+ Chapter 7: Spatiotemporal raster data handling with the `stars` package (available)  
+ Chapter 8: Creating publication-quality maps (wait a bit)  
	* use the `tmap` and `ggplot2` packages to create maps
+ Chapter 9: Download and process publicly available spatial datasets (partially available)
	...
	* Cropland Data Layer (`cdlTools`) - under construction
