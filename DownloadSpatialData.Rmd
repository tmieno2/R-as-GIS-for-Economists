...
+ any spatial objects as an `sf` or `sfc` object - `type = "b"`
...
Let's download the 2018 CDL data for the area that covers the Champaign, Vermilion, Ford, and Iroquois county in Illinois (a map below).
...
When you use an `sf` object for `aoi`, CDL data will be downloaded for the bounding box (this is why `type = "b"`) that encompasses the entire geographic area of the `sf` object irrespective of the type of the objects in the `sf` object (whether they are points, polygons, lines). In this case, CDL data is downloaded for the red area in the map below. 
...
The following code makes a request to download the CDL data for the Champaign county in Illinois in 2018. 
...
In the above code, the FIPS code for the Champaign county (17019) was supplied to the `aoi` option. Because a county is used here, the `type` argument is specified as 'f'. 
...
The downloaded raster data itself is not readily usable immediately for your economic analysis. It is typically that the variable of interest is the frequency of land use types or their shares. You can use `raster::freq()` to get the frequency (the number of raster cells) of each land use type.
...
`NoData` in `Crop` corresponds to the black area in the above figure, which is the portion of the raster data that does not overlap with the boundary of the Champaign county. These points with `NoData` can be removed by using the `filter` function. 
...
Let's see an example. The following codes request data on acreage changes in land cover for the Champaign county (FIPS = 17019) from 2017 (`year1 = 2017`) to 2018 (`year2 = 2018`). 
